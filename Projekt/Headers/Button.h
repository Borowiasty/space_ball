...
public:
    Button(const std::vector <sf::Font>& fonts, const std::map<std::string, sf::Texture>& textures, const std::string text, const int size, int width, int height, const int position_x, const int position_y)
    {
    ...
    {
        if (getGlobalBounds().contains({ static_cast<float> (mouse_pos.x), static_cast<float> (mouse_pos.y) })) { return true; }
        else { return false; }
    }
